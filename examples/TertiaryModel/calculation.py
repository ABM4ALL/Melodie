...
def calculate_mean_power_positive_values(value_array):
    list_filter = [value for value in value_array if value > 0]
    list_calc = []
    for item in list_filter:
        if item < 1:
            list_calc.append(item)
        else:
...
class EndUseAnalyzer(ABC):
    ...
    def print_info(self):
        ...
        print("sector_sample_selected_size = " + str(analyzer.sector_sample_selected_size))
        print("penetration_rate = " + str(analyzer.penetration_rate))
        print("adoption_sum = " + str(analyzer.adoption.sum()))
        print("duration = " + str(analyzer.duration))

        print("adoption_employee_cons_coef = " + str(analyzer.adoption_employee_cons_coef))
        print("adoption_employee_employee_coef = " + str(analyzer.adoption_employee_employee_coef))
        print("adoption_employee_cons_pvalue = " + str(analyzer.adoption_employee_cons_pvalue))
        print("adoption_employee_employee_pvalue = " + str(analyzer.adoption_employee_employee_pvalue))
        print("adoption_employee_average = " + str(analyzer.adoption_employee_average))

        print("adoption_floorarea_cons_coef = " + str(analyzer.adoption_floorarea_cons_coef))
        print("adoption_floorarea_floorarea_coef = " + str(analyzer.adoption_floorarea_floorarea_coef))
        print("adoption_floorarea_cons_pvalue = " + str(analyzer.adoption_floorarea_cons_pvalue))
        print("adoption_floorarea_floorarea_pvalue = " + str(analyzer.adoption_floorarea_floorarea_pvalue))
        print("adoption_floorarea_average = " + str(analyzer.adoption_floorarea_average))
        print()
...
class VehicleAnalyzer(EndUseAnalyzer):
    ...
    def set_adoption(self) -> np.array:
        adoption = (self.sector_sample_selected[end_use["car_number"]].values +
                    self.sector_sample_selected[end_use["delivery_van_number"]].values +
                    self.sector_sample_selected[end_use["truck_number"]].values)
        return adoption

    def set_duration(self) -> float:
        duration = (self.sector_sample_selected[end_use["car_distance_km_per_year"]].values +
                    self.sector_sample_selected[end_use["delivery_van_distance_km_per_year"]].values +
                    self.sector_sample_selected[end_use["truck_distance_km_per_year"]].values).sum()
...
class LightingAnalyzer(EndUseAnalyzer):
    ...
    def set_adoption(self) -> np.array:
        fluorescent_power = calculate_mean_power_positive_values(survey_data[end_use["fluorescent_average_power_kw"]].values)
        incandescent_power = calculate_mean_power_positive_values(survey_data[end_use["incandescent_average_power_kw"]].values)
        halogen_power = calculate_mean_power_positive_values(survey_data[end_use["halogen_average_power_kw"]].values)
        LED_power = calculate_mean_power_positive_values(survey_data[end_use["led_average_power_kw"]].values)

        adoption_kw = self.sector_sample_selected[end_use["lighting_total_capacity_kw"]].values
        areas = ["shop", "production", "office", "warehouse", "basement", "canteen", "other"]
        fluorescent_adoption = np.array([0 for i in range(0, self.sector_sample_selected_size)])
        incandescent_adoption = np.array([0 for i in range(0, self.sector_sample_selected_size)])
        halogen_adoption = np.array([0 for i in range(0, self.sector_sample_selected_size)])
        LED_adoption = np.array([0 for i in range(0, self.sector_sample_selected_size)])
        for area in areas:
            area_operating_share = self.sector_sample_selected[building["operation_in_" + area + "_share"]].values/100
            area_fluorescent_share = self.sector_sample_selected[end_use[area + "_lighting_fluorescent_share"]].values/100
            area_incandescent_share = self.sector_sample_selected[end_use[area + "_lighting_incandescent_share"]].values/100
            area_halogen_share = self.sector_sample_selected[end_use[area + "_lighting_halogen_share"]].values/100
            area_led_share = self.sector_sample_selected[end_use[area + "_lighting_led_share"]].values/100

            area_adoption_kw = adoption_kw * area_operating_share
            area_area_fluorescent_adoption = area_adoption_kw * area_fluorescent_share / fluorescent_power
            area_incandescent_adoption = area_adoption_kw * area_incandescent_share / incandescent_power
            area_halogen_adoption = area_adoption_kw * area_halogen_share / halogen_power
            area_LED_adoption = area_adoption_kw * area_led_share / LED_power

            fluorescent_adoption = fluorescent_adoption + area_area_fluorescent_adoption
            incandescent_adoption = incandescent_adoption + area_incandescent_adoption
            halogen_adoption = halogen_adoption + area_halogen_adoption
            LED_adoption = LED_adoption + area_LED_adoption
