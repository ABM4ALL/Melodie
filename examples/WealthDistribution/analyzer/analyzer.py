...
from Melodie import create_db_conn, Analyzer


class GiniAnalyzer(Analyzer):
    def run(self):
        scenarios = self.all_scenarios()
        for index, scenario in scenarios.iterrows():
            scenario_id = scenario['id']
            for step in range(int(scenario['periods'])):
                agent_result = self.agent_result('agent_list', scenario_id, step=step)
                print(agent_result,agent_result.mean())
        pass
