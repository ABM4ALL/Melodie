...
class Plotter(ABC):
    ...
    def setup_agent_strategy_convergence_heatmap_fig_params(self):
        self.agent_strategy_convergence_heatmap_prefix = (
            "agent_strategy_convergence_heatmap"
        )
    ...
    def setup_agent_strategy_convergence_lines_fig_params(self):
        self.agent_strategy_convergence_lines_prefix = (
            "agent_strategy_convergence_lines"
        )
    ...
    def setup_trainer_env_var_across_scenarios_fig_params(self):
        self.trainer_env_var_across_scenarios_prefix = (
            "trainer_env_var_across_scenarios"
        )
    ...
    def save_fig(self, fig, fig_name):
        fig.savefig(
            self.fig_folder + "/" + fig_name + ".png",
            dpi=self.fig_dpi,
            format=self.fig_format,
        )
        plt.close(fig)

    def trainer_agent_strategy_params_convergence_heatmap(
        self,
        ...
        figure = plt.figure(
            figsize=self.agent_strategy_convergence_heatmap_fig_size,
            dpi=self.fig_dpi,
            frameon=False,
        )
        ...
        if strategy_params_labels != None:
            y_ticks = strategy_params_labels

        ylim = self.check_if_input_none_use_default(
            ylim, self.agent_strategy_convergence_heatmap_vlim
        )
        ax = sns.heatmap(
        ...
        ax.set_xlabel(
            self.agent_strategy_convergence_heatmap_x_label,
            fontsize=self.x_label_fontsize,
            labelpad=self.x_labelpad,
        )
        ax.set_ylabel(
            self.agent_strategy_convergence_heatmap_y_label,
        ...
        fig_name = self.add_suffix_if_exist(
            self.agent_strategy_convergence_heatmap_prefix, fig_suffix
        )
        self.save_fig(figure, fig_name)

    def trainer_agent_strategy_params_convergence_lines(
        self,
        ...
        figure = plt.figure(
            figsize=self.agent_strategy_convergence_lines_fig_size,
            dpi=self.fig_dpi,
            frameon=False,
        )
        ...
        ) in strategy_params_agent_generation_matrix_dict.items():
        ...
        ylim = self.check_if_input_none_use_default(
            ylim, self.agent_strategy_convergence_lines_ylim
        )
        ax.set_ylim(ylim)
        ax.grid(self.grid)

        if len(strategy_params_labels) > 1:
            ncol = self.check_if_input_none_use_default(
                legend_ncol, self.calc_legend_ncol(len(strategy_params_labels))
            )
            figure.legend(
                fontsize=15,
        ...
        ax.set_xlabel(
            self.agent_strategy_convergence_lines_x_label,
            fontsize=self.x_label_fontsize,
            labelpad=self.x_labelpad,
        )
        ax.set_ylabel(
            self.agent_strategy_convergence_lines_y_label,
            fontsize=self.y_label_fontsize,
            labelpad=self.y_labelpad,
        )
        ...
        fig_name = self.add_suffix_if_exist(
            self.agent_strategy_convergence_lines_prefix, fig_suffix
        )
        self.save_fig(figure, fig_name)

    def trainer_env_var_evolution_lines(
        self,
        var_name: str,
        var_value_dict: dict,
        y_label: str = None,
        ...
        figure = plt.figure(
            figsize=self.env_var_evolution_fig_size, dpi=self.fig_dpi, frameon=False
        )
        ...
        ax.set_ylabel(
            self.check_if_input_none_use_default(y_label, var_name),
            fontsize=self.x_label_fontsize,
            labelpad=self.x_labelpad,
        )
        ax.set_xlabel(
            self.env_var_evolution_x_label,
            fontsize=self.y_label_fontsize,
            labelpad=self.y_labelpad,
        )
        ...
        if len(var_value_dict) > 1:
            ncol = self.check_if_input_none_use_default(
                legend_ncol, self.calc_legend_ncol(len(var_value_dict))
            )
            figure.legend(
                fontsize=15,
                bbox_to_anchor=(0, 1.02, 1, 0.1),
        ...
        fig_name = self.add_suffix_if_exist(
            self.env_var_evolution_lines_prefix, fig_suffix
        )
        self.save_fig(figure, fig_name)

    def trainer_agent_vars_scatter(
        self,
        var_1_name,
        var_1_value,
        var_2_name,
        ...
        figure = plt.figure(
            figsize=self.agent_vars_scatter_size, dpi=self.fig_dpi, frameon=False
        )
        ...
        ax.set_xlabel(
            self.check_if_input_none_use_default(var_1_label, var_1_name),
            fontsize=self.x_label_fontsize,
            labelpad=self.x_labelpad,
        )
        ax.set_ylabel(
            self.check_if_input_none_use_default(var_2_label, var_2_name),
            fontsize=self.y_label_fontsize,
            labelpad=self.y_labelpad,
        )

        if var_1_lim != None:
            ax.set_xlim(var_1_lim)
        if var_2_lim != None:
            ax.set_ylim(var_2_lim)
        ax.grid(self.grid)
    ...
    def trainer_env_var_across_scenarios_line(
        self,
        ...
        ax.set_ylabel(y_label, fontsize=self.x_label_fontsize, labelpad=self.x_labelpad)
        ax.set_xlabel(x_label, fontsize=self.y_label_fontsize, labelpad=self.y_labelpad)
        ...
        fig_name = self.add_suffix_if_exist(
            self.trainer_env_var_across_scenarios_prefix, fig_suffix
        )
        self.save_fig(figure, fig_name)
