# -*- coding:utf-8 -*-
# This file will not auto run in pytest command
...
def get_executable(venv_path: str) -> str:
    if sys.platform.lower().find('windows'):
        ...
        executable = os.path.join(venv_path, 'Scripts\python')
    return executable
...
def venvpackup():
    ...
    os.system(f'{executable} -m pip install {package}')

    os.chdir(os.path.dirname(__file__))
    os.system(f'{executable} -c "import Melodie.boost as boost;import Melodie.boost.hello; print(dir(boost));boost.b"')
    # print(proj_root)
    os.chdir(proj_root)
