...
def test_ast_manipulator():
    am = FuncDefManipulator(os.path.join(os.path.dirname(__file__), 'demo.py'), 'setup')
    ...
    assert " " * 8 + "self.add_agent_property('agents', 'id_xxxxxxx')" in unparsed
    assert " " * 8 + "self.add_agent_property('agents', 'cash')" not in unparsed
    assert " " * 8 + "self.add_agent_property('agents', 'fine')" not in unparsed
