...
def test_trainer_params():
    data = [{'id': 0, 'number_of_path': 1, 'number_of_generation': 1, 'strategy_population': 10, 'mutation_prob': 0.02,
             'strategy_param_code_length': 1, 'p_max': 1, 'p_min': 0}]
...
def test_calibrator_params():
    ...
    record = pd.DataFrame(data)
    gcp_2 = GACalibratorParams.from_dataframe_record(record.to_dict(orient='records')[0])
