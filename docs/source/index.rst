...
* Second, you can continue with the :ref:`Tutorial` section, in which we explain how an example model (`CovidContagion <https://github.com/ABM4ALL/CovidContagion>`_) is developed with Melodie step by step. It is a minimum example but shows a clear structure and the most important components to develop an ABM with **Melodie**.
* Third, for those who work with `Mesa <https://github.com/projectmesa/mesa>`_ or `AgentPy <https://github.com/JoelForamitti/agentpy>`_, we provide a comparison between **Melodie** and the two packages in the :ref:`Framework Comparison` section. The comparison is done based on the CovidContagion model in the tutorial.
