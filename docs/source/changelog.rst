...
- Add thread-based execution support to ``Simulator.run_parallel()`` via the ``parallel_mode`` parameter.
- Add thread-based parallel execution mode for ``Calibrator`` and ``Trainer`` via the ``parallel_mode`` parameter.
