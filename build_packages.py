...
def has_built_package(version: str, plat_name: str, arch: str):
    ...
    for package_name in os.listdir("archive"):
        ...
        print(
            version,
            version_str,
            plat.startswith(prefix),
            plat.endswith(suffix),
            version == version_str,
        )
        # time.sleep(10)
...
def build_process(os_name, arch, version, conda_name):
    ...
    if os.path.exists("dist"):
        shutil.rmtree("dist")
    cmd = (
        CONDA_ACTIVATION + f"conda activate {conda_name} && "
...
def build(d):
    ...
    for os_name, os_content in d.items():
        ...
        for interpreter_item in os_content:
            arch, version, conda_name = (
                interpreter_item["arch"],
                interpreter_item["version"],
                interpreter_item["conda-name"],
            )
